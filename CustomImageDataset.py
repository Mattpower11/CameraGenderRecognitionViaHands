...
class CustomImageDataset(Dataset):
    def __init__(self, data_structure, image_dir, id_exp, palmar_dorsal, transform=None):

        self.labels = {}

        if palmar_dorsal == 'dorsal':    
            self.image_filenames = np.array([riga[1] for riga in data_structure[id_exp]['images']]).flatten()
        else: 
            self.image_filenames = np.array([riga[0] for riga in data_structure[id_exp]['images']]).flatten()

        for x in range(0, len(self.image_filenames)):
            self.labels[self.image_filenames[x]] = data_structure[id_exp]['labels'][x]
